...
function findElementGreaterThan(arr, num) {
  ...
  const result = arr.find((elem, index, array) => elem > num)
  ...
  // Якщо ні повертаємо рядок Числа яке задовільняе умову не знайдено
    else {return "Числа яке задовільняе умову не знайдено";}
  }
...
function findIndexByProperty(arr, name) {
  ...
  // Використовуємо метод findIndex для пошуку індексу об'єкта, властивість 'name' якого співпадає з заданим іменем.
   const result = arr.findIndex((elem, index, array) => elem.name === name)
   if (result === -1)
   {return - 1;}
...
function findLastEvenNumber(arr) {
  ...
  const result = arr.findLast((elem, index, array) => elem % 2 === 0)
...
function getPositiveArrayValues(arr) {
  ...
  const valuesArray = Array.from(iter);
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  const result = valuesArray.filter((item) => item > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const newArr = arr.splice(startIdx);
...
function wordLengths(arr) {
  ...
  const result = arr.map (elm => elm.length)
...
const result = arr.splice(index, 0, element);
...
function expandArrayByN(arr, n) {
  // Перевіряємо, чи вхідні параметри є масивом та числом відповідно, 
  //якщо ні повертаємо 'Перший вхідний параметр має бути масивом, другий - числом'
  if (Array.isArray(arr) === false) 
  ...
  const result = arr.flatMap(elem => {
    // const string = String(elem)
    // const m = String(elem).repeat(n);
    // //console.log(m);
    // const num = Number(m);
    // //console.log(num);
    // Array.from(m);
    return Array.from(String(elem).repeat(n)).map(el => el = Number(el))
  });    
...
function findDuplicateElements(arr) {
  ...
  const result = arr.filter((elem, index, array) => 
...
function capitalizeWords(arr) {
  ...
  // Перетворюємо першу літеру слова у верхній регістр да додамо до неї всі символи слова крім першого
   ...
   const newArr = Array.from(elem)[0].toUpperCase() + elem.slice(1);
...
function calculateTotalPrice(arr) {
  ...
  const sum = arr.reduce((accum, elem) => accum + elem.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  ...
  const result = arr.findLastIndex(elem => elem < 0) 
